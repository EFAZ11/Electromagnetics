...
n = 120; r = 0.01; % number and radius of disks
...
z = [];
for j = 1:n
  z = [z; c(j)+rr(j)*circ]; % collocation points
end
...
for j = 1:n
  ...
  for k = 1:N
    zck = (zz-c(j)).^(-k); kk = k+(j-1)*N;
    uu = uu+a(kk)*real(zck)+b(kk)*imag(zck); 
  end
end
...
for j = 1:n
  disk = c(j)+rr(j)*z; fill(real(disk),imag(disk),[1 .7 .7])
  hold on, plot(disk,'-r'), 
end
